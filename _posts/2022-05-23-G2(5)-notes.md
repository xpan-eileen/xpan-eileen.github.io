...
A quick computation shows that this is indeed the case in our example $G_2(5)$. But what about in general, do we still have the intersections for $G_2(p)$ for other odd primes $p$? For this we want to see from the algebraic groups point of view. The general idea is that for a subgroup we recognise as an intersection in the finite group senario, we want to show that there are no larger subgroups containing it that can be an intersection of conjugate maximal subgroups of $G_2$.
